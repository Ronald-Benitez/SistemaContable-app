...
@section('content')
    ...
    <div class="container mt-5 pt-4">
        ...
        <form action="{{ route('Costos.update', $registro) }}" method="POST">
            ...
            <input type="number" name="monto" min="0.00 " step="0.01" value="{{ $registro->monto }}" id="nombre"
                class="form-control">
            ...
            <select class="form-select" name="type" id="tupoMonto">
                @php
                    $options = ['MD:  Material directo', 'MOD: Mano de obra directa', 'CIF', 'Periodo'];
                    $values = ['MD', 'MOD', 'CIF', 'Periodo'];
                    $output = '';
                    for ($i = 0; $i < count($options); $i++) {
                        $output .= '<option ' . ($registro->type == $values[$i] ? 'selected="selected"' : '') . ' value="' . $values[$i] . '">' . $options[$i] . '</option>';
                    }
                    echo $output;
                @endphp
            ...
            <div class="modal-footer">
                <a class="btn btn-secondary" href="{{ url()->previous() }}" data-bs-dismiss="modal">Cerrar</a>
